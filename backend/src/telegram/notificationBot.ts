...
export const sendOrderNotification = async (order) => {
    try {
        ...
        const itemsDetails = order.items.map(item => {
            ...
            const addonsList = item.selectedAddons.map(addon => 
                `├─ ${addon.name} (${addon.quantity} x ${addon.price} руб.)`
            ).join('\n') || '└─ Без допов';

            // Удаленные ингредиенты
            const removedList = item.removedIngredients.map(ing => 
                `├─ Не добавлять: ${ing.name}`
            ).join('\n') || '└─ Все ингредиенты';
...
${addonsList}
${removedList}
🔄 Итого за позицию: ${(item.price * item.quantity) + item.selectedAddons.reduce((sum, a) => sum + (a.price * a.quantity), 0)} руб.
            `;
        }).join('\n------------------------\n');
