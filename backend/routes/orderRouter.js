...
const {sendOrderNotification} = require('../telegram/notificationBot')
const { editOrder } = require("../controllers/order");
...
router.patch("/:id", async (req, res) => {
  try {
    const { error, order } = await editOrder(req.params.id, req.body);

    if (error) {
      return res.status(400).json({ error });
    }

    res.status(200).json("Статус заказа успешно обновлен", order);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
const orderItems = basket.items.map((item) => {
    ...
    await sendOrderNotification(order, phone)
